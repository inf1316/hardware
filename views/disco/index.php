...
/* @var $searchModel app\models\DiscoSearch */
...
<div class="disco-index">
    ...
    <p>
        <?php #echo Html::a('<i class="glyphicon glyphicon-plus"></i> Crear Disco', ['create'], ['class' => 'btn btn-success']) ?>
    </p>
    ...
    <?= GridView::widget([
        ...
        'panel' => [
            'heading' => '<h3 class="panel-title"><i class="glyphicon glyphicon-th-list"></i> Disco</h3>',
        ...
        'rowOptions' => function ($model) {
            $sql = 'select * from disco where referencia =:referencia';
            if (count(Disco::findBySql($sql, [':referencia' => $model->iddisco])->all()) > 0) {
                return ['class' => GridView::TYPE_DANGER];
            } else {
                list($dia, $mes, $año) = split('[/.-]', $model->fecha_disco);
        ...
        'columns' => [
            [
                ...
                'detail' => function ($model, $key, $index, $column) {
                    $searchModel = new app\models\_DiscoSearch();
                    $dataProvider = $searchModel->search(Yii::$app->request->queryParams, $model->iddisco);
    ...
    $js = "$('#deleteSelected').on('click', function () {
                pks = $('#disco').yiiGridView('getSelectedRows');
                if (pks.length != 0) {
