...
<div class="cpu-index">
    ...
    <p>
        <?php #echo Html::a('<i class="glyphicon glyphicon-plus"></i> Crear CPU', ['create'], ['class' => 'btn btn-success']) ?>
    </p>
    ...
    <?= GridView::widget([
        ...
        'panel' => [
            'heading' => '<h3 class="panel-title"><i class="glyphicon glyphicon-th-list"></i> CPU</h3>',
        ...
        'rowOptions' => function ($model) {
            $sql = 'select * from cpu where referencia =:referencia';
            if (count(Cpu::findBySql($sql, [':referencia' => $model->idCpu])->all()) > 0) {
                return ['class' => GridView::TYPE_DANGER];
            } else {
                list($dia, $mes, $año) = split('[/.-]', $model->fecha_cpu);
        ...
        'columns' => [
            [
                ...
                'detail' => function ($model, $key, $index, $column) {
                    $searchModel = new \app\models\_CpuSearch();
                    $dataProvider = $searchModel->search(Yii::$app->request->queryParams, $model->idCpu);
    ...
    $js = "$('#deleteSelected').on('click', function () {
                pks = $('#cpu').yiiGridView('getSelectedRows');
                if (pks.length != 0) {
