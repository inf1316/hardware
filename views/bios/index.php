...
/* @var $searchModel app\models\BiosSearch */
...
<div class="bios-index">
    ...
    <p>
        <?php #echo Html::a('<i class="glyphicon glyphicon-plus"></i> Crear Bios', ['create'], ['class' => 'btn btn-success']) ?>
    </p>
    ...
    <?= GridView::widget([
        ...
        'rowOptions' => function ($model) {
            $sql = 'select * from bios where referencia =:referencia';
            if (count(Bios::findBySql($sql, [':referencia' => $model->idBios])->all()) > 0) {
                return ['class' => GridView::TYPE_DANGER];
            } else {
                list($dia, $mes, $año) = split('[/.-]', $model->fecha_bios);
        ...
        'columns' => [
            [
                ...
                'detail' => function ($model, $key, $index, $column) {
                    $searchModel = new app\models\_BiosSearch();
                    $dataProvider = $searchModel->search(Yii::$app->request->queryParams, $model->idBios);
    ...
    $js = "$('#deleteSelected').on('click', function () {
                pks = $('#bios').yiiGridView('getSelectedRows');
                if (pks.length != 0) {
